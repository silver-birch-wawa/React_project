...
class Container extends Component{
   constructor(props){
	...
	/* 
	{"result":1,"information":{"workspace_cn":"电子科技大学",
	"name_pinyin":"zuozhe1","address":"四川省成都市","education":1,
	"gender":1,"academicsec1":1,"workspace_en":"UESTC","postcode":"610000","academicsec3":null,"phonenum":"13666666666","academicsec2":null,"title":"高级医师",
	"officetel":"副主任","major":1,"name":"作者1","location":1,"id":3,"researchdir":"临床医学","email":"23121@qq.com","introduction":"就是六"}}
	*/
	...
	*/
	...
	this.icon['银行卡号']=<Icon style={{marginRight:'6px'}} type="credit-card"/>
	this.icon['再输一遍新密码']=<img class='img-icon' src={require('../img/passwd.png')}/>;
	//this.subjects=['外科','内科','妇科']
	this.subjects=[];

	this.gender={1:'男',2:'女'};
	this.password=['',''];
	this.reflect={'gender':'性别','name':'姓名','性别':'gender',
			'姓名':'name','address':'收寄地址','收寄地址':'address','phonenum':'联系方式',
			'researchdir':'研究方向','postcode':'所在地邮编','education':'学历','introduction':'个人介绍','officetel':'办公室电话','card':'银行卡号'};
	}
	componentDidMount(){
			// 角色判断
...
//		console.log(value);
	...
	handleClick(e){
		if(this.password[0]!=''){
			if(this.upload['再输一遍新密码']!=this.password[1]){
				alert('您输入的新密码不一致');
				return;
			}
		}
		delete this.upload['再输一遍新密码'];
		let password={};
		console.log(this.upload);
		 	...
		 	<html style={{paddingLeft:'35px'}}>
				<middleForm>
				  ...
				  <div class='form-section' style={{border:'1px bold black'}}>
				    {this.icon['性别']}
				    <p>性别：&nbsp;</p>
				    <Select onChange={this.handlechange.bind(this,{'item':'性别'})} labelInValue defaultValue={{key:this.Personal_info['性别']}} style={{width:'4rem'}}>
				  ...
				  <div class='form-section'>
				    {this.icon['密码']}
				    <p>老密码：&nbsp;</p>
					<Input style={{width:'20%'}} onChange={this.handlechange.bind(this,{'item':'老密码'})}/>
				    &nbsp;&nbsp;
				    <p>新密码：&nbsp;</p>
					<Input style={{width:'20%'}} onChange={this.handlechange.bind(this,{'item':'新密码'})}/>	
				    &nbsp;&nbsp;
				    <p>再输一遍新密码：&nbsp;</p>
					<Input style={{width:'20%'}} onChange={this.handlechange.bind(this,{'item':'再输一遍新密码'})}/>										
				  </div>

				  {
					...
					if(item=='学历'){
						return(<div class='form-section' style={{border:'1px bold black'}}>
						    {this.icon['学历']}
						    <p>学历：&nbsp;</p>
						    <Select onChange={this.handlechange.bind(this,{'item':'学历'})} labelInValue defaultValue={{key:this.state.education[this.Personal_info['学历']-1]}} style={{width:'6rem'}}>
							    {
